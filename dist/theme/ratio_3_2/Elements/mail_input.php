	...
	foreach ($mailFields as $key => $record) {
		...
		if ($field['use_field'] && ($blockStart && $iteration >= $blockStart) && (!$blockEnd || $iteration <= $blockEnd)) {
			...
			if ($group_field != $field['group_field'] || (!$group_field && !$field['group_field'])) {
				...
				echo $this->Mailform->label("MailMessage." . $field['field_name'] . "", $field['head']);
				if ($field['not_empty']) {
			...
			if (!$field['group_valid']) {
				echo $this->Mailform->error( "MailMessage." . $field['field_name'], null, array( 'class' => 'alert alert-danger' ) );
			}
			...
			if (($this->BcArray->last($mailFields, $key)) ||
				...
				if ($field['group_valid']) {
					if ($field['valid']) {
						echo $this->Mailform->error( "MailMessage." . $field['group_field'], "この項目は必ずご入力ください。", array( 'class' => 'alert alert-danger' ) );
					}
					echo $this->Mailform->error( "MailMessage." . $field['group_field'] . "_not_same", "入力内容が一致していません。", array( 'class' => 'alert alert-danger' ) );
					echo $this->Mailform->error( "MailMessage." . $field['group_field'] . "_not_complate", "入力内容に不明な点があります。", array( 'class' => 'alert alert-danger' ) );
				}
